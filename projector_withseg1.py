...
@click.option('--colors', type=bool, help='Gen colors for color interpolation', default=False, show_default=True)
...
#----------------------------------------------------------------------------
# save colors
# def save_colors(G, w, c, outdir):
#     G = copy.deepcopy(G).eval().requires_grad_(False).to(device) # type: ignore
#     synth_image = G.synthesis(w.unsqueeze(0).to(device), c=c, noise_mode='const')['image']
#     synth_image = (synth_image + 1) * (255/2)
#     synth_image = synth_image.permute(0, 2, 3, 1).clamp(0, 255).to(torch.uint8)[0].cpu().numpy()
